...
export default function CharacterSheet({sheet}){
  ...
  return <div className="space-y-4 print:bg-white print:text-black">
    ...
    <div className="grid lg:grid-cols-2 gap-5">
      <Card className="p-5 print:shadow-none print:border-black print:bg-white"><h3 className="font-black text-2xl mb-3">Quirk</h3><div className="flex gap-2 flex-wrap mb-3"><Badge>Ataque {fmt(d.quirkAttack)}</Badge><Badge>CD {d.quirkDc}</Badge><Badge>Atributo: {sheet.quirk.attribute}</Badge><Badge>{expectedTech} técnica(s)</Badge><Badge>{expectedStrat} estratagema(s)</Badge><Badge>Grau máx. {maxGrade}</Badge><Badge>PVirtuais {Number(sheet.quirk.virtualPointsUsed||0)}/{Number(sheet.quirk.virtualPointsMax||0)}</Badge><Badge>Livres {Math.max(0, Number(sheet.quirk.virtualPointsMax||0)-Number(sheet.quirk.virtualPointsUsed||0))}</Badge></div><Label title="Conceito" text={sheet.quirk.concept}/><Label title="Manifestações" text={sheet.quirk.manifestations}/><Label title="Estratagemas" text={sheet.quirk.strategies}/><Label title="Pontos Virtuais — notas" text={sheet.quirk.virtualPointsNotes}/><Label title="Limites/Fraquezas" text={sheet.quirk.weakness}/><div className="mt-3"><div className="text-sm uppercase tracking-wide text-zinc-500 print:text-black">Habilidades Nativas</div><div className="space-y-2 mt-2">{(sheet.quirk.nativeAbilities||[]).map(id=>{const n=NATIVE_ABILITIES.find(x=>x.id===id); return <div key={id} className="rounded-xl border border-zinc-800 bg-zinc-950 p-4 print:border-black print:bg-white"><b>{n?.name || id}</b><p className="text-sm text-zinc-300 print:text-black whitespace-pre-wrap">{n?.exactText || n?.summary || '—'}</p></div>})}</div></div><details className="mt-3 text-base"><summary className="cursor-pointer font-bold">Pontos Virtuais das Manifestações</summary><p className="mt-2 whitespace-pre-wrap text-zinc-300 print:text-black">{VIRTUAL_POINTS_TEXT}</p></details></Card>
    ...
    <Card className="p-5 print:shadow-none print:border-black print:bg-white"><h3 className="font-black text-2xl mb-3">Legado, personalidade e história</h3><div className="grid md:grid-cols-2 gap-5"><Label title="Legado" text={`${legacy?.name || '—'}
${legacy?.exactText || legacy?.summary || ''}`}/><Label title="Notas do legado" text={sheet.legacyNotes}/><Label title="Personalidade" text={sheet.personality}/><Label title="Sonho/Caminho" text={`${sheet.dream || '—'} / ${sheet.path || '—'}`}/><Label title="Backstory" text={sheet.backstory}/><Label title="Aparência" text={sheet.appearance}/></div></Card>
